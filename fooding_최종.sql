...
COMMENT ON COLUMN "QUESTION"."Q_NO" IS '문의번호(PK)';
...
COMMENT ON COLUMN "FAQ"."FAQ_NO" IS 'FAQ번호(PK)';
...
COMMENT ON COLUMN "NOTICE"."NOTICE_NO" IS '공지사항번호(PK)';
...
COMMENT ON COLUMN "RESTAURANT"."RES_NO" IS '업체번호';
...
COMMENT ON COLUMN "RESTAURANT"."CELPHONE" IS '휴대폰 번호';
...
COMMENT ON COLUMN "MONTHLY"."MON_NO" IS '정산번호';
...
COMMENT ON COLUMN "NOT_ABLE"."NOT_NO" IS '스케줄번호';
...
CREATE TABLE "ATTACHMENT" (
	...
	"BOARD_TYPE"	VARCHAR2(1)		NOT NULL CHECK(BOARD_TYPE IN('N','R','Q'))
);
...
COMMENT ON COLUMN "ATTACHMENT"."BOARD_TYPE" IS '(공지사항 N / 리뷰 R / 1:1문의 Q)';
...
CREATE TABLE "MENU" (
	...
	"MENU_DES"	VARCHAR2(2000)		NOT NULL,
...
COMMENT ON COLUMN "TB_USER"."USER_NO" IS '회원번호(PK)';
...
COMMENT ON COLUMN "BOOK_MENU"."BM_NO" IS '주문번호';
...
CREATE TABLE "SITE" (
	...
	"WRITE_TYPE"	VARCHAR2(1)		NOT NULL CHECK(WRITE_TYPE IN('B','P','I'))
);

COMMENT ON COLUMN "SITE"."WRITE_TYPE" IS '(배너 B / 이용약관 P / 개인정보처리방침 I)';
...
COMMENT ON COLUMN "BLACK"."BLACK_NO" IS '신고번호';
...
-- 업체 시퀀스
CREATE SEQUENCE SEQ_RESNO START WITH 1 INCREMENT BY 1;
-- 리뷰 시퀀스
